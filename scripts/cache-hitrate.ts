...
// As src/metrics.rs reads it: exactly the empty string is unset, and `off` in any case,
// untrimmed, means nothing was recorded, so there is nothing to read.
const rawEnv = process.env.XWEN_METRICS_FILE;
const fileEnv = rawEnv === undefined || rawEnv === "" ? undefined : rawEnv;
