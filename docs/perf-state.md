...
- **The Flash-Next prefill rows were measured beside 77 GB of weights, where the current
  default file measures 93.2 GB resident** (2026-09-17). The 131424 row's 31.0 GB peak had
  16 GB of GPU working set that a run today does not. Since the same day Flash-Next's
  chunk narrows with the cache past 49,152 positions — 2048, then 1024, then 512 above
  98,304 — so the 131424 row is no longer the shape that produced it, and its prefill rate
  at the narrower chunk is unmeasured (records/gpu-working-set.md). Flash-Next's rows above
  49k are due a retake; the 35B-A3B's are not, it being the architecture the taper
