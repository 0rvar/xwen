...
- [Kernel policy](decisions/kernel-policy.md)
  ...
  - The prefill chunk narrows with the context past the sparse gate, the hoisted causal…
...
- [Metrics](decisions/metrics.md)
  ...
  - `serve` keeps its operational lines in a bounded `serve.log`, and only `serve`…
- [Tokenization, chat, tool calls](decisions/tokenization-chat-tool-calls.md)
  ...
  - Qwen3 gets two chat dialects and no tool calling.
...
- [Z-Image-Turbo](decisions/zimage.md)
  - candle's `z_image` module is vendored into `src/zimage/`, not depended on in place and…
  - Text conditioning comes from `XwenModel::encode`, never from candle's own text encoder.
  ...
  - The transformer runs bf16 end to end. fp16 is REFUTED, and fp8 and Q8_0 are not taken.
  - Verification is a torch dump with an injected latent, gated at the step-0 velocity…
  - First image before first bar.
  ...
  - The encoder and the transformer are both resident for the whole run.
  - The sigma grid is diffusers', and the official pipeline computes the same one.
  - An out-of-range RoPE position is refused, not clamped, and `check_size` grew a third…
  - A PNG is written to a temporary sibling and renamed.
  - A path that names a diffusion snapshot is refused at the `CheckpointSource` seam, and…
  - An agreement bar is bracketed from both sides or it is not a bar.
  ...
  - The SwiGLU dual gemm is REFUTED: two weight planes against one activation in one…
...
- [Qwen-Image 2.1](decisions/qwen-image.md)
  ...
  - A prompt past 4096 tokens is refused, not truncated.
  - The scheduler is its own file, not a second arm inside the vendored one.
  - Block-causal attention runs as segments on the existing kernels, with no dense mask…
  - The prefix K/V cache is on by default, and running without it is a bisect arm, not a…
  - The parity bars are Z-Image's, and both wrong graphs fall outside them.
  - The encoder gate's bars are set from the reference's own spread, and kept row 0 has a…
  - The image is returned RGBA only when at least 10 pixels have alpha at most 8.
  - The u8 rounding of exact ties is left as it is.
  - Memory admission for this model is a line through two measured peaks, and the 1 MP…
  ...
  - Drains are per stage on the xwen arm and per convolution on the candle arm.
  ...
  - The text encoder loads per request on serve and is not kept warm.
  - `--image-steps` is Z-Image's alone.
  - Image models are listed on `GET /v1/images/models`, not on `/v1/models`.
  - `negative_prompt` and `guidance_scale` are 400s on both pipelines.
  - Request faults are classed by whose fault they are.
