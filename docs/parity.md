...
| `XWEN_DUP_STAGE=<names>` / `XWEN_DUP_REPS=<n>` | duplicate-dispatch probe: re-encodes the named stages' kernel launches `XWEN_DUP_REPS` extra times (default 1) on every multi-token call, dropping the copies' results, so the prefill wall-clock difference against a plain run is that stage's GPU time IN SITU, divided by the rep count. `<names>` is a comma-separated list of `experts`, `experts_down`, `moe_glue`, `shexp`, `hc`, `hc_gemm`, `gdn`, `gdn_scan`; an unknown name is a hard error at the first probed dispatch, never an ignored entry. Decode (`n == 1`) never repeats. Read the delta as a LOWER bound for a stage that leaves the GPU idle: the copies write fresh buffers and candle's encoder barriers only on hazards, so they can overlap the original | a measurement INSTRUMENT, not a kill switch: the wrapped launchers are pure functions of tensors the caller already holds, the first (real) result is the one returned, and every copy is discarded, so the arithmetic is identical set or unset. Unlike the profilers it adds no syncs. Stripped by `parity-gate.ts` via an `XWEN_DUP_` prefix match, and by `flashnext-replay.ts`'s blanket `XWEN_*` strip; a stray one would only multiply a gate run's prefill time |
