...
fix batch that touched map0), verbatim both times:
`PASS strict (classic mv fallback) code-short cos=1.000000 top5=5/5` ·
`PASS mm code-short cos=0.999631 top5=5/5` ·
`PASS decode code-short 63/64 agree, 1 excused, 0 mismatch` ·
`PASS decode text-mixed 63/64 agree, 1 excused, 0 mismatch` ·
`PASS decode long-mixed 62/64 agree, 2 excused, 0 mismatch` ·
`PASS ppl corpus Δnll=0.000791` · `ALL PASS (6 graded)`. Digit-identical to the
0261e17 run, as the bitwise test predicts (the grid and the width decide which
...
| side | env |
|---|---|
| Reference (all tiers) | `XWEN_ATTN_F32=1 XWEN_ATTN_MM_CLASSIC=1 XWEN_COMBINE_CLASSIC=1 XWEN_ATTN_GLUE_CLASSIC=1 XWEN_FLASH_CLASSIC=1 XWEN_ACT_CLASSIC=1 XWEN_DELTA_CLASSIC=1 XWEN_DENSE_MM_CLASSIC=1 XWEN_MV_EXT_CLASSIC=1 XWEN_MOE_GLUE_CLASSIC=1 XWEN_MOE_SHEXP_CLASSIC=1 XWEN_ROUTER_MV_CLASSIC=1`, `--moe-impl reference` |
| strict candidate | the first eight, PLUS `XWEN_MV_EXT_CLASSIC=1 XWEN_MOE_SHEXP_CLASSIC=1 XWEN_ROUTER_MV_CLASSIC=1 XWEN_NO_MM_ID=1 XWEN_MV_CLASSIC=1`, `--moe-impl fused` — i.e. every switch of the Reference row except `XWEN_MOE_GLUE_CLASSIC` (the fused glue is bit-identical and is what strict grades), plus the two that force the classic expert mat-vec |
...
More switches exist and appear in NO row of that table, because they affect
...
`XWEN_HC_GATE_CLASSIC`, `XWEN_HC_GATE_FUSED_MAX_N`,
`XWEN_QSA_CLASSIC`, `XWEN_QSA_HOST_TOPK`, `XWEN_QSA_HOST_MASK`, `XWEN_QSA_TIMER`, `XWEN_QSA_ATTN_CLASSIC`, `XWEN_QSA_TILE`, `XWEN_QSA_SPARSE_MIN_KV`, `XWEN_QSA_UNION_STATS`, `XWEN_HOST_MASK` and `XWEN_PLE_PROFILE` from the run env (6eaf980 for the first;
...
|---|---|---|
| `XWEN_HC_CLASSIC=1` | reverts all nine fused hyper-connection kernels — `kernel_hc_norm`, `_norm_inject`, `_norm_split`, `_inject`, `_silu_quarter`, `_mix`, `_write` and the decode gate's `_gate_down` / `_gate_up_mix`, counting entry points in hc.metal rather than logical steps — to the candle chains they replace — it is the outer of the two hc switches, and `XWEN_HC_GATE_CLASSIC` reverts the gate alone | a real kill switch, and the one to pin on a reference side once qwen4exp is gradeable: `hc_silu_quarter` and `hc_write` are bit-identical to the chains but `hc_norm`, `hc_mix` and both gate kernels are BOUNDED, the same class as `XWEN_DELTA_CLASSIC` |
...
| `XWEN_QSA_HOST_MASK=1` | reverts the QSA indexer's device-side prefill selection and mask (`ops::qsa_select::select_mask`, `kernel_qsa_select_mask`: one threadgroup per query, the decode kernel's radix top-k per row, the `[n, n_kv]` additive mask written in place) to the score-plane readback, host top-k and host-filled mask upload it replaced on 2026-09-06; `XWEN_QSA_CLASSIC` implies it | non-numeric: the same mask bits, pinned by `device_mask_matches_host_mask_bitwise`; `bun scripts/flashnext-replay.ts --control XWEN_QSA_HOST_MASK=1` is its replay |
| `XWEN_QSA_TIMER=1` | times the host arm's round trip (readback, selection, fill, upload) per above-budget prefill chunk per QSA layer and prints the totals after the prefill; inert on the device arm | instrumentation; adds a sync per round trip on the host arm |
| `XWEN_QSA_ATTN_CLASSIC=1` | reverts the tile-batched sparse prefill attention on QSA layers (`ops::qsa_tiles`, `qsa_tiles.metal`: per tile of `XWEN_QSA_TILE` queries the union of selected blocks, its K/V rows and mask gathered, one batched sdpa over the tiles) to dense sdpa over the whole cache under the `[n, n_kv]` mask, the route until 2026-09-06; implied by `XWEN_QSA_CLASSIC` and `XWEN_QSA_HOST_MASK` | NUMERIC, and it belongs in a gate row once the harness runs Flash-Next: the sparse route sums the same terms in a different order, so it is held to the dense route at f16 output tolerance (`sparse_tiles_reproduce_the_dense_mask_route`) and to llama.cpp by the forced replay, not bitwise |
| `XWEN_QSA_TILE=<n>` | queries per tile on that route (multiple of 32; default `ops::QSA_TILE_DEFAULT`) | a shape knob: any value is the same math to summation order |
| `XWEN_QSA_SPARSE_MIN_KV=<n>` | cache length at or above which the sparse tile route runs (default `ops::QSA_SPARSE_MIN_KV_DEFAULT`, 49152; `0` everywhere above budget) | a length gate between two routes that agree to summation order; below it the dense route, which is the bitwise one |
| `XWEN_QSA_UNION_STATS=1` | one stderr line per above-budget prefill chunk per QSA layer with the union sizes per tile size (the instrument that chose the tile) | instrumentation; reads the mask back |
...
| `XWEN_HC_GATE_CLASSIC=1` | reverts the FUSED DECODE GATE — `kernel_hc_gate_down` and `kernel_hc_gate_up_mix`, which fold the grouped norm, the injection head, BOTH q8_0 bottleneck projections, the activation and the mix into two dispatches (three per gate with the write-back, two on the tail mixer) — to the seven-dispatch split path: the split norm pair, the two `QLinear` matmuls, `hc_silu_quarter` and `hc_mix`. Taken at `n <= XWEN_HC_GATE_FUSED_MAX_N` only, so prefill is untouched either way | a real kill switch, bounded class like `XWEN_HC_CLASSIC`, and the `--control` arm for `scripts/flashnext-replay.ts`. Both fused dots reassociate: the down rows fold per-thread `simd_sum` partials where the gemv folds its own partition, the stream mean is a `simd_shuffle_xor` butterfly where `hc_mix` runs a serial loop, and the per-stream statistics are partitioned per q8_0 block. The REFERENCE bound is what holds it: rel_l2 <= 1e-5 against `ref_hc` (`gate_fused_matches_reference`, `gate_fused_covers_every_admitted_shape`). Against the split path the same tests grade 1e-6 at one token and 5e-5 above it, looser only because the split path's `QMatMul` stages half activation tiles past one row and sits ~1.7e-5 from the oracle where the gate sits ~1.6e-7. Flash-Next only and not gradeable until qwen4exp has a tier. Provenance `hc_gate` (schema v10, grandfather "classic"); no tier pins it — pin it with `XWEN_HC_CLASSIC` when the checkpoint becomes gradeable. Stripped by name |
| `XWEN_HC_GATE_FUSED_MAX_N=<n>` | token count at or below which a hyper-connection read takes the fused decode gate (default 8; `0` keeps every batch on the split path, a large value pushes the fused gate into prefill counts it was not shaped for). INCLUSIVE, unlike `XWEN_HC_SPLIT_MAX_N` — the gate covers `n == 1`. `0` is a second kill switch by another name, and provenance `hc_gate` reads "classic" for it: the label and the read path ask one predicate (`ops::hc_gate_fused_enabled`) | an A/B knob for the threshold, but NOT a neutral one the way `XWEN_HC_SPLIT_MAX_N` is: the two arms it chooses between are bounded-close, not bit-identical, so moving it moves the same numbers `XWEN_HC_GATE_CLASSIC` does. Stripped by name |
...
| `XWEN_PLE_TAIL_CLASSIC=1` | restores the host f32 PLE gate, signed sqrt, gated norm, dilated conv and silu for Metal forwards with `n > 1`, in place of the device kernels (`ops/ple.metal`, the default since 2026-09-05); decode always runs the host tail | a real kill switch and the `--control` arm for `scripts/flashnext-replay.ts`. Bounded, not bitwise: the simdgroup-partitioned f32 reductions differ from the host's sequential/f64 sums at ulp scale (6e-7 abs on real inputs). Stripped by `parity-gate.ts`. On long-mixed step 4 the device tail flips a decision the host tail held by 0.31 logit, which the engine near-tie rule excuses (log.md 2026-09-05, gate and conv) |
...
| `XWEN_HOST_MASK=1` | restores the HOST-built causal prefill mask — the scalar double loop over `seq x (pos + seq)` plus its upload — in place of the device build (`PrefillMask::causal_on_device`, the default since 2026-09-06). A non-Metal device takes the host path whether or not this is set | a real kill switch and the `--control` arm for `scripts/flashnext-replay.ts`, but the two arms are BIT-IDENTICAL by construction and by test (`the_device_built_causal_mask_equals_the_host_fill` compares both the additive f32 plane and the f16 sdpa copy at every chunk shape the prefill walk produces), so it can never move a parity number. Every checkpoint. Stripped by `parity-gate.ts` by name |
...
| `XWEN_SHEXP_QMATMUL=1` | keeps the MoE shared expert's three q8_0 projections on candle's `QMatMul` at every token count, instead of the vendored dense cooperative-tensor gemm (`QLinear::forward_gemm` → `ops::matmul_dense_q`) above `dense_mm_min_seq`. Since the fused shared expert landed this moves nothing at 1..=8 tokens, the same way `XWEN_HC_GEMM_QMATMUL` moves nothing there: inside `XWEN_MOE_SHEXP_FUSED_MAX_N` the pair computes all three projections itself and reads none of them through `QLinear`, so this switch reaches the shexp planes only above the ceiling or with `XWEN_MOE_SHEXP_CLASSIC` set — which is also where its prefill target lives, so the switch loses nothing it was for | the same kernel and precision class as the 27B's dense FFN (~3.7e-4 rel_l2 from the `QMatMul` route at the shexp shapes, `forward_gemm_matches_qmatmul_at_prefill_and_is_forward_below`), and pinned off the same way: `forward_gemm` honours `XWEN_DENSE_MM_CLASSIC`, which both sides of **strict** already set, so strict never sees it and mm / decode / ppl grade it. `XWEN_SHEXP_QMATMUL=1` restores exactly the immediate pre-change route — which already included the mv_ext 2..=8-token window, the shexp planes predating this change. Provenance `shexp_gemm` (schema v9, grandfather "classic"). Stripped by name |
| `XWEN_MOE_SHEXP_CLASSIC=1` | reverts the FUSED SHARED EXPERT — `kernel_moe_shexp_gate_up` (gate gemv + up gemv + SwiGLU + the `ffn_gate_inp_shexp` logit, one dispatch) and `kernel_moe_epilogue_shexp` (the block epilogue with the shexp DOWN gemv folded in) — to the five-dispatch chain: two `QLinear` gemvs, `ops::silu_mul`, a third gemv and a candle f32 gemv, feeding the plain `ops::moe_epilogue`. Four launches per MoE layer, so 192 of a Flash-Next decode token's ~1356 and 160 on the 35B-A3B. Taken at `n <= XWEN_MOE_SHEXP_FUSED_MAX_N` only, so prefill is untouched either way | a real kill switch and NOT a bit-identity anchor, unlike the rest of the moe_glue family: all three fused dots reassociate (the gate/up rows fold per-thread `simd_sum` partials where `QMatMul`'s gemv folds its own partition, and the shexp-aware epilogue's routed combine folds over a full simdgroup where `kernel_moe_epilogue` folds over `next_pow2(top_k/2)` lanes). `kernel_moe_epilogue` ITSELF is untouched and still bitwise wherever this path is off, which is what keeps strict's anchor intact. Bound: rel_l2 <= 1e-5 against an f32 host reference and 1e-6 / 2e-6 (n = 1 / n > 1) against the classic chain (`shexp_fused_matches_reference`, four geometries including the two that make the kernels' strided partitions run, measured maxima 3.1e-7 and 5.9e-7). The cross-path gap is that small because the classic arm never reaches `QMatMul`'s f16-tile matmul inside this window: the shexp projections are planed, so they run `QMatMul`'s f32 gemv at one token and the vendored `ops::matmul_mv_ext` at 2..=8, and both arms are f32 reassociations of the same dots — they sit LEVEL against the f32 oracle (ratio 0.95..1.11), which is why the test asserts levelness rather than an ordering. Pinned "classic" on both sides of **strict** (referenceEnv and the strict candidate env), and DECODE is the tier that grades it — mm and ppl are prefill and run above the ceiling, where provenance labels the configured route the way `dense_mm` does on the 35B. `XWEN_MOE_GLUE_CLASSIC=1` disables it too: that switch takes the block off the epilogue path this lives in. The `moe_shexp` label comes from the ENVIRONMENT predicate (`ops::moe_shexp_fused_enabled`) and not from a dispatch count, so a run where the expert runner declined to hand its projection over — `ExpertFfn::project` returning `None`, as the f16-staging rescale branch does under an `XWEN_MM_ID_MIN_SEQ` override — is labelled "fused" while the shared expert actually ran classic; the default tiers never take that branch at or below the ceiling. Provenance `moe_shexp` (schema v11, grandfather "classic"). Stripped by name |
| `XWEN_MOE_SHEXP_FUSED_MAX_N=<n>` | token count at or below which an MoE block's shared expert takes the fused pair (default 8; `0` keeps every batch on the five-dispatch chain, a large value pushes the pair into prefill counts it was not shaped for — where the three projections would otherwise take the dense cooperative-tensor gemm). INCLUSIVE, like `XWEN_HC_GATE_FUSED_MAX_N`: the pair covers `n == 1`. `0` is a second kill switch by another name, and provenance `moe_shexp` reads "classic" for it: the label and `MoeBlock::forward` ask one predicate (`ops::moe_shexp_fused_enabled`) | an A/B knob for the threshold, but NOT a neutral one: the two arms it chooses between are bounded-close, not bit-identical, so moving it moves the same numbers `XWEN_MOE_SHEXP_CLASSIC` does. Stripped by name |
| `XWEN_ROUTER_MV_CLASSIC=1` | reverts the MoE ROUTER PROJECTION — the vendored f32 gemv (`ops::matmul_f32`, `kernel_mul_mv_f32_f32_v` in `src/ops/f32.metal`) over the `[n_expert, hidden]` `ffn_gate_inp` plane — to candle's `Tensor::matmul` over the `[hidden, n_expert]` transpose the block also holds. What it buys: candle's mlx `gemv_t` tile pick gives a one-token router product EIGHT threadgroups for the whole 5.24 MB plane on Flash-Next (the 35B-A3B's 2048 x 256 lands on 8 as well), where the gemv runs `ceil(n_expert/2) x t`, 256 at 512 experts — 252 MB of router weight per Flash-Next decode token moved on an occupied kernel instead of an idle one. Taken at `n <= XWEN_ROUTER_MV_MAX_N` only, so prefill is untouched either way | a real kill switch and NOT a bit-identity anchor. Both arms multiply the same f32 operands and accumulate in f32 — only the SUMMATION ORDER differs (four simdgroups' partials folded through threadgroup memory, against mlx's 8-lane shuffle ladder over 320-term serial partials) — so the logits are bounded-close (rel_l2 <= 1e-5 against a sequential host f32 dot AND against the candle arm, `matmul_f32_matches_host_reference`, measured 2026-09-06 at both production geometries and t = 1/3/8: 8.0e-7..9.8e-7 against the host dot, 3.1e-7..9.3e-7 against candle's METAL matmul — the arm the kernel actually replaces, which is what that test now builds both operands on). The reason it still cannot sit under a bitwise tier is downstream, not upstream: routing is top-k over those logits and top-k is DISCRETE, so a near-tie between two experts can flip the selection outright. UNLIKE the `moe_shexp` row, no outer switch covers this one — the router projection runs BEFORE the routing decision, so `--moe-impl reference` and `XWEN_MOE_GLUE_CLASSIC` both still dispatch it, which is why the oracle's env pins this switch explicitly. Pinned "classic" on both sides of **strict** (referenceEnv and the strict candidate env); mm / decode / ppl grade the gemv, decode being the tier that dispatches it at its production token count. Provenance `router_mv` (schema v12, grandfather "classic"; values mv/classic). Stripped by name |
| `XWEN_ROUTER_MV_MAX_N=<n>` | token count at or below which an MoE block's router projection takes the vendored f32 gemv (default 8; `0` keeps every batch on candle's matmul). INCLUSIVE, like `XWEN_MOE_SHEXP_FUSED_MAX_N`: the gemv covers `n == 1`. A value ABOVE 8 cannot widen the window — the kernel re-reads the whole plane per token row, so `ops::matmul_f32` refuses past 8 and `MoeBlock::router_mv` keeps candle there. `0` is a second kill switch by another name, and provenance `router_mv` reads "classic" for it: the label and `MoeBlock::route` ask one predicate (`ops::router_mv_enabled`) | an A/B knob for the threshold, but NOT a neutral one: the two arms it chooses between are bounded-close, not bit-identical, so moving it moves the same numbers `XWEN_ROUTER_MV_CLASSIC` does. Stripped by name |
| `XWEN_HC_GEMM_QMATMUL=down\|up\|both` | keeps the named hyper-connection bottleneck projection(s) (`hc_*_down` [320,10240] and `hc_*_up` [10240,320], q8_0) on `QMatMul` at prefill instead of the dense gemm; unset routes both above `dense_mm_min_seq`, from the fused hc read only. The hc planes are dense_mm-only (`QLinear::without_mv_ext`, decisions.md), so at 2..=8 tokens no hc path opens the mv_ext window; that leaves those tokens bitwise on QMatMul only when the fused decode gate is off (`XWEN_HC_CLASSIC`, `XWEN_HC_GATE_CLASSIC`, or `XWEN_HC_GATE_FUSED_MAX_N` below the token count) — by default 1..=8 tokens take the gate, which computes both projections itself and reads neither through `QLinear` | two arms because `up` is a ten-NK-step shape whose win is a measurement; the same precision class and the same `XWEN_DENSE_MM_CLASSIC` pin as the shexp row (~3.7e-4 rel_l2 at both shapes, same test). Not gradeable until qwen4exp has a tier. Provenance `hc_gemm` (schema v9, grandfather "classic"; values classic/fused/down-only/up-only). Stripped by name |
| `XWEN_DUP_STAGE=<names>` / `XWEN_DUP_REPS=<n>` | duplicate-dispatch probe: re-encodes the named stages' kernel launches `XWEN_DUP_REPS` extra times (default 1) on every multi-token call, dropping the copies' results, so the prefill wall-clock difference against a plain run is that stage's GPU time IN SITU, divided by the rep count. `<names>` is a comma-separated list of `experts`, `experts_down`, `router_proj`, `moe_glue`, `shexp`, `hc`, `hc_gemm`, `gdn`, `gdn_scan`, `sdpa` (the multi-token attention kernel, both architectures), `qsa_score`, `qsa_mask` (the QSA block scoring, and the device mask plus its f16 copy); an unknown name is a hard error at the first probed dispatch, never an ignored entry. NOTE on `shexp` with the fused shared expert (`XWEN_MOE_SHEXP_CLASSIC` unset, token count within `XWEN_MOE_SHEXP_FUSED_MAX_N`): the shared expert's DOWN projection is inside `kernel_moe_epilogue_shexp`, so its cost is priced under `moe_glue`, not `shexp`, and `shexp` covers only the gate/up/SwiGLU/logit kernel; a probe above the ceiling still sees the classic five-dispatch split. Decode (`n == 1`) never repeats unless `XWEN_DUP_DECODE` is also set, which prices a decode stage by the same difference (with more room under the lower bound: a decode chain leaves the GPU idle more often). Read the delta as a LOWER bound for a stage that leaves the GPU idle: the copies write fresh buffers and candle's encoder barriers only on hazards, so they can overlap the original. At decode that bound is the rule rather than the corner case (measured 2026-09-06, log.md): a delta above zero is a FLOOR for the stage and a delta of about zero means it overlaps its own copy, never that it is free, so the probe cannot price a latency-bound decode stage | a measurement INSTRUMENT, not a kill switch: the wrapped launchers are pure functions of tensors the caller already holds, the first (real) result is the one returned, and every copy is discarded, so the arithmetic is identical set or unset. Unlike the profilers it adds no syncs. Stripped by `parity-gate.ts` via an `XWEN_DUP_` prefix match, and by `flashnext-replay.ts`'s blanket `XWEN_*` strip; a stray one would only multiply a gate run's prefill time |
...
XWEN_MV_EXT_CLASSIC=1 XWEN_MOE_GLUE_CLASSIC=1 XWEN_MOE_SHEXP_CLASSIC=1 \
XWEN_ROUTER_MV_CLASSIC=1 ./target/release/logits-dump --model "$M" --moe-impl reference \
...
and decode is a single token, so no fixture produces a forward inside the window. One
`QLinear` site left that window when the fused shared expert landed: at 1..=8 tokens the
pair computes the shexp gate/up/down itself and reads none of the three through
`QLinear`, so `XWEN_MV_EXT_CLASSIC` moves those planes only with `XWEN_MOE_SHEXP_CLASSIC`
set or above `XWEN_MOE_SHEXP_FUSED_MAX_N` — the same carve-out the
`XWEN_HC_GEMM_QMATMUL` row states for the hc planes, and for the same reason. The
...
`unsloth/Qwen3-4B-GGUF/Qwen3-4B-BF16.gguf`, generated by `bun scripts/qwen3-fixtures.ts`.
The test asserts xwen's ids equal the oracle's and that `decode(ids)` returns the text.
It runs under `cargo test`, CPU-only, and self-skips when the HF cache has no Qwen3
tokenizer. Valid only for NFC input, which the generator enforces (decisions.md "The HF
tokenizer normalizes to NFC").

**Chat template.** `bun scripts/verify_chat_template_qwen3.ts` starts its own
`llama-server --jinja -ngl 0`, posts to `/apply-template` and compares byte for byte:
16/16 on 2026-09-06 and still 16/16 after the model-opened-thinking work, 12 base-dialect
cases and 4 Instruct. Two branches the endpoint cannot reach, because llama-server
preprocesses the conversation before the template sees it: a conversation ending in an
assistant message is treated as a prefill, and assistant content holding an inline
`<think>` block is eaten by the server's own reasoning parser. Both are covered by named
tests in `src/chat.rs` against the vendored jinja instead.

### The Stage 1 oracle

`scripts/llama-logits-all.cpp`, built by `scripts/build-llamacpp.sh` beside the other
oracle binaries, links libllama and decodes an id file with `batch.logits[i]` set on
every token, streaming raw f32 `[n_tokens, n_vocab]` to disk. Neither existing oracle
gives per-position logits: eval-callback computes them for the last position only, and
`--kl-divergence-base` is uint16-compressed with a 16-logit floor. A JSON sidecar records
backend, `n_gpu_layers`, KV types, batch geometry, flash-attn, threads, the GGUF sha256
and the llama.cpp commit, plus llama.cpp's own `load_tensors:` lines, since no public API
reports which devices a loaded model actually used. `bun scripts/qwen3-ref-logits.ts`
drives it over the fixture prompts into a manifest and asserts each output's size. The
20-prompt CPU run took 55.6 s and produced 3.83 GB.

```
bash scripts/build-llamacpp.sh          # builds llama-logits-all with the rest
bun scripts/qwen3-ref-logits.ts --out-dir /tmp/qwen3-ref-logits-cpu   --n-gpu-layers 0
bun scripts/qwen3-ref-logits.ts --out-dir /tmp/qwen3-ref-logits-metal --n-gpu-layers 99
```

Both arms exist and both are graded against, because they are not the same computation.
llama.cpp's CPU path narrows F32 activations to BF16 before every BF16 matmul: the type
traits set `vec_dot_type = GGML_TYPE_BF16`, `ggml_compute_forward_mul_mat` converts src1
through `from_float` whenever the types differ, and the llamafile fast path has no ARM
branch, so it declines and the narrowing always runs. The Metal path keeps F32
activations against BF16 weights, which is what xwen does, so the Metal arm is the closer
reference. KV type f16 on both sides. How far apart the two arms are is measured below,
and it turned out to be the most important number in the track.

### Running the three gates

Stage 1, the full-vocabulary logits, `#[ignore]` and env-keyed:

```
XWEN_QWEN3_PARITY_DIR=/tmp/qwen3-ref-logits-metal \
  cargo test --release --test qwen3_parity -- --ignored --nocapture
```

`--nocapture` is load bearing: the per-prompt table prints on a pass too, and cargo
swallows a passing test's stdout. A failure repeats the table inside the panic. Cheap
subset, prompts 8, 11 and 10 being 1, 8 and 16 tokens:
`XWEN_QWEN3_PARITY_ONLY=8,10,11`.
...
| `XWEN_QWEN3_PARITY_MAX_CTX` | default is the longest selected prompt; a lower value is refused, the gate never truncates |
| `XWEN_QWEN3_PARITY_LIST` | how many failing positions the panic spells out, default 50 |

Stage 2, the encoder hidden states:
...
`XWEN_ZIMAGE_DIR` overrides the `text_encoder/` directory (default the entry's cached
snapshot), `XWEN_ZIMAGE_ONLY` selects prompt indices, `XWEN_ZIMAGE_VERIFY_SHA=1` checks
each dump against the fixture digest. The gate is ordered and stops at the first failing
step, because a later step's number would be meaningless: rendered string byte-equal,
then ids after the 512 truncation, then `encode(ids, 35)` against the fp32 reference. It
...
taps onto `qwen3.cpp`'s as the identity plus the pre-o_proj `kqv_out-{il}`, and skips
`attn_o_proj`. The deciding fact, read in the source rather than inferred: in
`llm_graph_context::build_attn`, `cb(cur, "kqv_out", il)` fires on the output of
`build_attn_mha`, BEFORE `build_lora_mm(wo, ...)`, and the only cb after `wo` is
commented out. llama.cpp never names the post-o_proj value on this architecture, so our
`attn_o_proj` tap has no counterpart and must be skipped rather than matched.

### What the gates measured, 2026-09-07

`pmset -g` read `lowpowermode 2`; no high-power claim is made.

Stage 2 passes: minimum cosine 0.99999449 and maximum relative error 0.00388 over
positions 1 and up, position 0 at 0.99999955 and 0.00089, all twelve rendered templates
and id sequences byte-equal first. For scale, the pipeline's own bf16 execution scores
0.99960 and 0.03236 against the same reference, and the bf16 output cast alone accounts
for 0.003784 of xwen's 0.00388.

Stage 1 over all 20 prompts, 6307 positions:

| arm | pooled max-abs | argmax | pooled top-5 |
| --- | --- | --- | --- |
| xwen vs the CPU oracle | 0.379 | 6300/6307 | 99.9176% |
| xwen vs the Metal oracle | 0.222 | 6304/6307 | 99.9239% |
| the CPU oracle vs the Metal oracle | 0.358 | 6303/6307 | 99.9239% |

Every argmax flip on every arm is inside the 2e-2 near-tie band. Error does not grow with
...
- **The harness cannot run on Qwen3.8-Flash-Next (`qwen4exp`) at all** (2026-08-29).
  ...
  numbers in docs/qwen4exp-parity-2026-08-29.md). **Codified 2026-09-05 as
  `bun scripts/flashnext-replay.ts [--control <switch>=1]`**: it builds or reuses the
  llama-server oracle over the three committed fixtures, replays the candidate (and,
  with `--control`, the same binary with the change under test switched off), and
  grades 64 steps per fixture. A mismatch is excused as a near-tie when the ORACLE's
  margin over the candidate's pick is under the band (1.0 at q8 decode), OR when the
  CONTROL arm holds the oracle's token over the candidate's pick by under the band:
  the pre-change engine did not confidently make that decision either, so the flip
  is reassociation crossing a boundary. That second rule exists because on
  long-mixed step 4 (oracle margin 1.83) reversing the summation order of one host
  f32 dot product, with no other change, produced the identical hard mismatch
  (log.md 2026-09-05, gate and conv). Excuses cap at 8 per fixture; anything else
  is hard and fails. Two things follow even after the
...
- **The Qwen3 track does not compare against the identical file** (2026-09-06). Every
  tier above runs the oracle on the same GGUF the engine loads; there is no `qwen3` GGUF
  in xwen, so the oracle runs a BF16 GGUF conversion of the same upstream weights while
  the engine loads the HF safetensors. BF16 survives that conversion, so the tensors
  should be identical, but "should be" is the point: the conversion's faithfulness is
