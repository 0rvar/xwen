...
existing one. Architecture and bars: [qwen3-dense.md](qwen3-dense.md). As of 2026-09-06
the first two stages have run and the rest is built but not yet gradeable, there being no
layer stack to grade.
...
- Chat template. `bun scripts/verify_chat_template_qwen3.ts` starts its own
  ...
  16/16 on 2026-09-06, 12 base-dialect cases and 4 Instruct. Two branches the endpoint
  cannot reach, because llama-server preprocesses the conversation before the template
  sees it: a conversation ending in an assistant message is treated as a prefill, and
  assistant content holding an inline `<think>` block is eaten by the server's own
  reasoning parser. Both are covered by named tests in `src/chat.rs` against the vendored
  jinja instead.

**The Stage 1 oracle.** `scripts/llama-logits-all.cpp`, built by
`scripts/build-llamacpp.sh` beside the other oracle binaries, links libllama and decodes
an id file with `batch.logits[i]` set on every token, streaming raw f32
`[n_tokens, n_vocab]` to disk. Neither existing oracle gives per-position logits:
eval-callback computes them for the last position only, and `--kl-divergence-base` is
uint16-compressed with a 16-logit floor. A JSON sidecar records backend, `n_gpu_layers`,
KV types, batch geometry, flash-attn, threads, the GGUF sha256 and the llama.cpp commit,
plus llama.cpp's own `load_tensors:` lines, since no public API reports which devices a
loaded model actually used. `bun scripts/qwen3-ref-logits.ts --out-dir <dir>` drives it
over the fixture prompts into a manifest and asserts each output's size. The 20-prompt
CPU run took 55.6 s and produced 3.83 GB.
...
**The CPU arm is not the arithmetic being graded, and this is the reason the bar is not
committed to.** llama.cpp's CPU path narrows F32 activations to BF16 before every BF16
matmul: the type traits set `vec_dot_type = GGML_TYPE_BF16`, `ggml_compute_forward_mul_mat`
converts src1 through `from_float` whenever the types differ, and the llamafile fast path
has no ARM branch, so it declines and the narrowing always runs. The Metal path keeps F32
activations against BF16 weights, which is what xwen does. So the qualification run is
the Metal arm, run when the GPU frees; the record states which backend the bar was met
against and by how much, and only then does 2e-2 become a floor. KV type f16 on both
sides.

**The tests that will consume all this**, both `#[ignore]` and env-dir keyed like
`tests/parity.rs`: `tests/qwen3_parity.rs` (`XWEN_QWEN3_PARITY_DIR`) comparing
...
that test is where the gemv-versus-gemm and flash-versus-sdpa splits would show.

**Bisection.** The qwen3 stack publishes the same tap names as the GGUF stacks, which map
onto `qwen3.cpp` as the identity, plus a new pre-o_proj tap `kqv_out-{il}`; `attn_o_proj`
is not compared on this architecture because llama.cpp does not name the post-wo tensor.
`scripts/parity.ts` needs an `--arch qwen3` name table, its current `refTapNames` being
hardcoded to the 3.6 `(il+1)%4` rule.
