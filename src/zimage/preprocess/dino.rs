...
//! DINOv2 Small depth backbone, exact erf GELU and fixed trained position grid.
...
impl DinoVisionTransformer {
    ...
    fn interpolate_pos_encoding(&self, xs: &Tensor, w: usize, h: usize) -> Result<Tensor> {
        if w != IMG_SIZE || h != IMG_SIZE || xs.dim(1)? != self.pos_embed.dim(1)? {
            candle_core::bail!("depth backbone expects the trained 518 square position grid");
        }
        Ok(self.pos_embed.clone())
    }
