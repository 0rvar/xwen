...
reading after the probe priced its projection at zero; the threadgroup-count audit on
the Front is what would find the rest) and the syncs plus the serial scan.]

[Amended 2026-09-07, after the Z-Image-Turbo pipeline arc (log.md "`xwen image` renders
through Z-Image-Turbo"): three image entries join the Front and three decode entries are
demoted to make room, each of them the smallest measured gain in its class (+1.4%, +1.4%
and ~+4%). The image work is ranked the way the scope amendment says it should be
(decisions.md "Diffusion image transformers are in scope, held to correctness bars
first"): the parity gate and the waiting-user route rank high because they are
correctness and a person, while the image step time ranks BELOW every decode lever on the
...
[Amended 2026-09-08, after the rope-and-norm arc and the attention arc (log.md "Z-Image's
elementwise rows and its attention"): the two Z-Image Front items both shipped and moved to
the archive, the norm-with-scale area item shipped with them, and ONE item is promoted into
the gap of two, a Metal-4 tensor-op attention kernel. The Front is eight. The promotion is
the same rank the two it replaces held, below every decode lever on the three design-target
checkpoints, because images are not a tok/s target. Two things did NOT become items and the
reason is the intake rule rather than an oversight: `attn.qknorm` at ~0.5 s per image is a
record line, and the remaining distance from the gemms' 30-39 TFLOP/s to the ~70 peak is
still a record line. Triage of the Z-Image items: every one of them is dated 2026-09-07 or
later, so none is near the 30-day line. The merged profiler pass then repriced every image
row and RE-RANKED them, which is why the annotations below carry new numbers: the VAE conv
path is now the largest lever on the graph at 3.7-4.2 s per image, the SwiGLU f32 store is
second at 2.6 s, the Front's attention kernel third at 2.4 s, and a new unpriced item holds
the gemm fusion at ~2 s. The Front entry stays at position 6 rather than moving up, because
the two rows above it are area items by choice: the VAE is a whole conv path and the SwiGLU
store is a precision change the gate has to arbitrate, while the attention kernel is the one
of the three that is a bounded piece of kernel work. Taking every priced row puts a step at
~1.25 s and a render at ~11.5 s against 2.15 and 22.4
([the record](docs/records/zimage-perf.md), "Lever ledger").]

[Amended 2026-09-08, evening, after the VAE conv, tensor-op attention and SwiGLU store arcs
(log.md "Z-Image's VAE on a direct conv kernel and its attention on the tensor units"): the
Front's attention kernel shipped (e5d9775) and moved to the archive, the VAE conv path area
item shipped with it (a763c61), and the SwiGLU f32-store item is RETIRED to the archive
under "Retired: Image generation", its basis having been the profiler's pool eviction and
the built bf16 store measuring parity (decisions.md "The bf16 SwiGLU store is REFUTED").
ONE item is promoted into the gap and it is an instrument rather than a lever: a
`powermetrics` trace across a 1024x1024 run, because the clean figures on e5d9775 read
1.35 s on the first step rising to 2.0-2.1 s by step 8, a 55% ramp where it was 20%, and
until that is read no image lever has a ceiling at the plateau. Same rank as the item it
replaces, below every decode lever, images not being a tok/s target. The Front is eight.
Two Z-Image items carry fresh dated lines because their basis changed: the gemm fusion
(the four gemms are the whole large bucket now, and the microbench prices it) and the
elementwise tail (bounded by the residual at ~0.1-0.3 s a step). Not items, by the intake
...
[Amended 2026-09-21, after the Qwen-Image 2.1 text-to-image arc (log.md "Qwen-Image 2.1
renders from `xwen image`"): two items are promoted into the two free places and nothing is
demoted, so the Front is ten and FULL. Both are already in flight, which is why they sit at
9 and 10 and not where a waiting user would otherwise rank: the serve route for the model,
and the direct-conv arm for its VAE decode, which is the largest stage, the whole memory
peak and the gate on the native size. Four more items from the arc are area items under
...
- [ ] [measured] **The token-id readback sync (`stack.rs:511`): the host uploaded those ids one line
   ...
   NEW item, unstarted.
   DEMOTED from the Front 2026-09-07 to make room for the Z-Image arc. Kept: +1.4% for a
   change with no new kernel is still the cheapest thing on this ledger per hour spent.
  From: Flash-Next perf ledger, re-ranked from the measured budgets (2026-09-05, step 4).
...
- [ ] [measured] **GDN: 252 dispatches (13%), the three fusion candidates in the P3 ledger.** At 4 µs
   ...
   DEMOTED from the Front 2026-09-07 to make room for the Z-Image arc. Kept: the +1.4%
   merge is real but it needs an A/B before it is worth an arc's slot.
...
- [ ] [measured] **Above the 2048 indexer budget: +165 dispatches** ≈ 0.66 ms at 4 µs, most of the
   ...
   first).
   DEMOTED from the Front 2026-09-07 to make room for the Z-Image arc. Kept, not
   retired: ~+4% at every context past 2048 tokens is still the largest of the three
   demoted gains, and it is the one to promote back first.
  From: Flash-Next perf ledger, re-ranked from the measured budgets (2026-09-05, step 4).
...
- [ ] [unpriced] **Fuse Z-Image's q/k/v and SwiGLU gemms, then tune the tiles at those shapes.**
  ...
  isolation, so the shape is right and the question is dispatch count and tiling. Two
  fusions: q, k and v are one `[3840, 11520]` gemm instead of three, and the SwiGLU pair is
  one `[3840, 20480]` instead of two. Then tune the tiles for M around 4000 at N 11520 and
  N 20480, which no sweep has covered. Guess ~1.26 to ~1.0 s per step, so ~2 s per image, and
  it is UNPRICED because nothing has measured a fused gemm at those N yet. **Cheap to
  price**: `tests/zimage_microbench.rs` already runs these shapes. Note the overlap: the
  SwiGLU f32-store item below lives inside this bucket, so the two do not pay twice
  (2026-09-08).
  **2026-09-08, evening: re-based.** The four gemms are the whole large bucket now, attention
  having moved to the tensor units at 45 TFLOP/s isolated and the SwiGLU store item having
...
- [ ] [measured] **The four full-tensor copies around Z-Image's attention.**
  ...
  **2026-09-08: half of it is done, and the rest is folded in with the other small rows.**
  All four copies go through the single-pass permutes now and k/v are written at f16. On the
  merged tree all NINE elementwise and copy rows together are ~0.34 s of a 2.15 s step
  against ~0.15 s with epilogues and a fused per-head `attn.qknorm`, so ~1.5 s per image
  for the lot; no single row of them is worth an arc, which is why they are one ledger line.
  **2026-09-08, evening: bounded by the residual.** ~1.05 s profiled over nine rows on e5d9775
...
- [ ] [unpriced] **The image pad-token path: sizes whose token count is not a multiple of 32.**
  ...
  `(w/16) * (h/16)` is a multiple of 32, so 1024x1024, 1024x768, 512x512 and 1536x1024
  pass while 528x528 is refused at 1089 image tokens. The transformer's real rule is
  looser: `x_pad_token` exists so a ragged image grid can pad up to a multiple of 32,
  with the pad rows at rope position (0,0,0), participating in attention as ordinary keys
  and queries and NOT masked. Implementing it is a piece of math with no oracle behind it
  yet, so it waits on Stage 3 rather than shipping untested. The batched attention-mask
  path beside it is in the same state (2026-09-07).
  [Record](docs/records/zimage-pipeline.md).
  From: Deferred from the Z-Image-Turbo pipeline arc (2026-09-07, Arc A).

- [ ] [small] **Measure the image pipeline's footprint from a user shell.** `footprint`, `ps`
  and `vmmap` were all refused with "operation not permitted" from the agent sandbox, so
  the arc that shipped `xwen image` has no memory number at all. Expected from the loads:
  7.6 GB encoder, 12.3 GB bf16 transformer, 0.34 GB f32 VAE, so ~20 GB plus activations,
  with the fp32-to-bf16 cast transiently holding one f32 tensor. Five minutes with the GPU
  free, and the serve route has to plan around whatever it says (2026-09-07).
...
- [ ] [unpriced] **Qwen-Image 2.1 editing with reference images (plan Phase 5).**
  The owner put it in scope on 2026-09-21. Shaped for and not wired: `Layout::from_slots`
  and the ordered segment list, the rope walk per image block, `rope::
  mrope_interleaved_tables`, the VAE encoder, and `conditioning::render` writing the
  `<imageN>` markup while `prompt_ids` refuses a non-empty list. Not built: the Qwen2-VL
  image preprocessor, the 27-block vision tower with its four mergers, the pad-token scatter
  and DeepStack adds in the text tower, RGBA-aware image input, `--reference`, `references`
  on the native route. Derived cost, not measured: about +13% a step per 1 MP reference and
  2.15 GB of prefix K/V each. Start with the encoder on a reference-image prompt against a
  new dump stage, brackets MRoPE-not-applied and DeepStack-skipped. Read
  `preprocessor_config.json` facts from the record first (2026-09-21).
  [Plan](docs/qwen-image-2.1-plan.md), [reference](docs/qwen-image.md).
  From: Deferred from the Qwen-Image 2.1 text-to-image arc (2026-09-21).

- [ ] [blocked] **Image Studio support for Qwen-Image 2.1 (plan Phase 6).**
  A model picker, a reference-image list with tag chips, an "Edit" mode and per-model
